"""
Online benchmark — Warmup baseline (Llama-3.1-70B, Scenario A)

Provisions ALL available nodes into pipelines, no spot events.
ShuntServe pipelines (P1, P2) + extra pipelines for remaining nodes.

Remaining nodes (not in ShuntServe config):
  - spot_g5_12xlarge 1,2 → P3 (40+40 layers, pp=[4,4])
  - on_demand_g6e_xlarge 1,2,3,4 → P4 (20+20+20+20 layers, pp=[1,1,1,1])
"""
import asyncio
...
OUTPUT_DIR = os.path.join(SPOT_TOLERANCE_DIR, "results", "llama3-70b", "online", f"scenario_{SCENARIO}")
...
EXTRA_PIPELINES = [
    {
        ...
        "node_layer_mapping": [
            ["spot_g5_12xlarge_node_ip_1", 40],
            ["spot_g5_12xlarge_node_ip_2", 40],
        ],
    },
    {
        "label": "WU-P4",
        "predicted_throughput_rps": 0.3,
        "config": {
            "model_name": MODEL_NAME,
            "total_num_layers": 80,
            "gpu_memory_utilization": 0.85,
        ...
        "node_layer_mapping": [
            ...
            ["on_demand_g6e_xlarge_node_ip_2", 20],
            ["on_demand_g6e_xlarge_node_ip_3", 20],
            ["on_demand_g6e_xlarge_node_ip_4", 20],
        ],
    },
]
...
async def test_benchmark():
    ...
    logger.info("=" * 70)
    logger.info(f"Online Benchmark — Warmup (All Nodes) — Scenario {SCENARIO}")
    ...
    try:
        ...
        metrics = await run_trace_benchmark(
            ...
            trace_output_prefix=f"spottolerance_online_warmup_llama3_70b_scenario_{SCENARIO}",
