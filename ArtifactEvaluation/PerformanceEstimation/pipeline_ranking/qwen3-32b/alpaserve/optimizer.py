"""
AlpaServe Optimizer for Qwen3-32B: shows what pipeline config the DP optimizer
produces for each homogeneous instance group.
...
def run_alpaserve_optimizer(instance_type, num_instances, model_name, model_config, head_dim, gpu_mem_utilization=0.85):
...
def main():
    ...
    for i, (instance_type, num_instances, gpu_util) in enumerate(groups, 1):
        result = run_alpaserve_optimizer(instance_type, num_instances, model_name, model_config, head_dim, gpu_util)
