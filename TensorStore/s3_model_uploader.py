...
def save_tensor_to_file(tensor_name: str, tensor_data: torch.Tensor, output_dir: str, target_dtype: Optional[torch.dtype] = None) -> tuple[str, dict]:
    ...
    Returns:
        tuple: (file_path, profiling_stats)
            profiling_stats contains: serialize_time, file_size
    """
    ...
    logging.info(f"Saved {tensor_name}: {file_size / (1024**2):.2f} MB [serialize: {serialize_time:.3f}s]")
    ...
    return file_path, profiling_stats
    ...
    try:
        ...
        for tp_size in tp_sizes:
            for tp_rank in range(tp_size):
                ...
                # Save to local file
                local_file_path, profiling_stats = save_tensor_to_file(
