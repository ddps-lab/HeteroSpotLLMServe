"""
Offline benchmark — Warmup baseline (Llama-3.1-70B long-context / Mooncake, Scenario A)

Provisions ALL available nodes into pipelines, no spot events.
ShuntServe pipelines (P1, P2) + an extra pipeline for the remaining (spare) nodes.

Remaining nodes (not in the ShuntServe scenario_A config):
  - spot_g5_12xlarge 1,2          (restored-spot spare)
  - on_demand_g6e_xlarge 1,2,3,4  (failover spare for the g6e interruptions)

NOTE (long-context): unlike the AzureConversation warmup, the four single-GPU
g6e.xlarge nodes CANNOT host an even [20,20,20,20] full-model split at long context
(5745-token contexts overflow a 20-layer shard — num_blocks=0, infeasible). The spare
nodes are therefore combined into ONE pipeline WU-P3 = g5×2 (TP4) + g6e×4 (TP1). Its
partition is the OPTIMIZER's choice for this spare sub-cluster at the long-context
workload — run_test_case on {(spot)g5.12xlarge:2, (spot)g6e.xlarge:4}, only_throughput:
partition [14,14,13,13,13,13], num_gpu_blocks=14010, max_batch_size=37, ~0.192 req/s —
so all 15 physical nodes are warmed.
"""
import asyncio
...
SPOT_TOLERANCE_DIR = os.path.dirname(MODEL_DIR)              # MooncakeAgentTool/
...
# ─── Extra pipelines for remaining (spare) nodes ─────────────────────
# Long-context: the 6 spare nodes (g5×2 + g6e×4) are combined into ONE feasible
# pipeline (g6e cannot host a standalone full-model split at 16k context).
# num_gpu_blocks / max_batch_size computed with shuntserve_optimizer.Pipeline
# at the long-context workload (input 5745 / output 154, max_model_len 16384).

MODEL_NAME = "meta-llama/Llama-3.1-70B-Instruct"
S3_PATH = "s3://hetero-spot-llm-serve-models/meta-llama/Llama-3.1-70B-Instruct"
RUNTIME_MAX_MODEL_LEN = int(16384 * 1.1)  # 18022

EXTRA_PIPELINES = [
    {
        "label": "WU-P3",
        "predicted_throughput_rps": 0.192,
        "config": {
            "model_name": MODEL_NAME,
            "total_num_layers": 80,
            "gpu_memory_utilization": 0.85,
            "pp_layer_partition": "14,14,13,13,13,13",
            "parallel_strategy": [4, 4, 1, 1, 1, 1],
            "max_model_len": RUNTIME_MAX_MODEL_LEN,
            "max_num_batched_tokens": RUNTIME_MAX_MODEL_LEN,
            "max_num_seqs": 512,
            "model_source": "s3",
            "s3_path": S3_PATH,
            "num_gpu_blocks": 14010,
            "max_batch_size": 37,
        },
        "node_layer_mapping": [
            ["spot_g5_12xlarge_node_ip_1", 14],
            ["spot_g5_12xlarge_node_ip_2", 14],
            ["on_demand_g6e_xlarge_node_ip_1", 13],
            ["on_demand_g6e_xlarge_node_ip_2", 13],
            ["on_demand_g6e_xlarge_node_ip_3", 13],
            ["on_demand_g6e_xlarge_node_ip_4", 13],
        ],
    },
]
