...
class RayDistributedExecutor(DistributedExecutorBase):
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup",
        ...
        # Original vLLM code commented out; we skip worker reordering.
        ...
        # Use adjust_rank without reordering workers.
        # Reordering causes errors with heterogeneous parallelism.
        start_rank = 0 if self.use_ray_spmd_worker else 1
        ...
        self._run_workers("adjust_rank", rerank_mapping)
        # End of modified worker ordering code
        ...
        for rank, (node_id, _) in enumerate(worker_node_and_gpu_ids):
            ...
            is_driver_worker = (not self.parallel_config) # No parallelism or
            # Since workers are not yet initialized, parallel_config's global rank
            # is not set, so we must determine driver workers manually.
            # Otherwise every worker would consider itself the driver.
            if not is_driver_worker: # If parallel_config is present
                tp_rank = rank
        ...
        # Warning: enabling use_ray_spmd_worker will cause errors
        if self.use_ray_spmd_worker:
        ...
        # Modified code for uneven TP/PP partitioning
        # Populate driver/non-driver worker lists accordingly.
        ...
        # Remove the last element (sentinel)
        driver_global_ranks.pop()

        logger.info(f"driver_global_ranks: {driver_global_ranks}")

        for index, worker in enumerate(self.workers):
            # The rank-0 driver worker is not in self.workers (handled separately)
            rank = index + 1
