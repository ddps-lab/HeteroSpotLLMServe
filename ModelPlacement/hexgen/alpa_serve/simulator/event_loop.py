...
class EventLoop:
    ...
    async def run(self):
        while self.queue:
            ...
            # Python 3.12+ requires tasks, not coroutines
            pause_wait_task = asyncio.create_task(self.pause_event.wait())
            done, pending = await asyncio.wait([atask, pause_wait_task],
