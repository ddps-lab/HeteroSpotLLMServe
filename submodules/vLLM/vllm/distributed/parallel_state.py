...
# Store parallel_strategy globally to support is_first_stage property lookups
...
class GroupCoordinator:
    ...
    def send_full_tensor_and_broadcast(self, 
                                       tensor_dict: Dict[str, Union[torch.Tensor, Any]], 
                                       is_driver: bool):
        """Driver worker sends the full tensor dict to the next PP stage driver."""
        # Send logic is a no-op for non-driver workers
        if not is_driver:
            return

        if not torch.distributed.is_initialized():
            return
        # self.world_size does not work correctly for non-driver workers.
        # Use get_pp_size() to check PP group size instead.
        if get_pp_size() == 1:
            return

        # Compute the next rank in the PP group
        ...
        # Send metadata (reusing send_object via CPU group)
        # send_object's dst parameter is the rank within the group.
        ...
        # Send tensors (full, no slicing) via device group (NCCL)
        # torch.distributed.send's dst parameter is the global rank
        for tensor in tensor_list:
            if tensor.numel() == 0:
                continue
            # Send full tensor
    ...
    def recv_full_tensor_and_broadcast(self, 
        ...
        """
        Driver worker receives metadata and tensors from the previous PP stage,
        then broadcasts to non-driver workers in its TP group.
        Non-driver workers receive the broadcast from the driver and return.
        """
        if not torch.distributed.is_initialized():
        ...
        if not is_driver:
            ...
            # Non-driver worker receives broadcast from driver and returns
            if tp_group.world_size > 1:
                ...
                raise ValueError("A non-driver with TP group size 1 should not exist")
            return received_tensor_dict
        
        # === DRIVER WORKER LOGIC START ===
        # Driver worker receives metadata and tensors from previous PP stage driver
        # Then broadcasts received data to its TP group
        src_pp_rank_in_group = (self.rank_in_group - 1) % self.world_size

        src_global_rank = self.ranks[src_pp_rank_in_group]
        
        # Receive metadata (reusing recv_object via CPU group)
        # recv_object's src parameter is the rank within the group.
        # It internally converts to global rank for the receive.
        ...
        # Receive tensors (full, no slicing) via device group (NCCL)
        # torch.distributed.recv's src parameter is the global rank
        received_tensor_dict = {}
        for key, value in metadata_list:
            if isinstance(value, TensorMetadata):
                # Use metadata device info instead of worker device info
                ...
                if not tensor.is_cuda and torch.distributed.get_backend(self.device_group) == "nccl":
                        # Receiving CPU tensors over NCCL would cause an error.
                        # Should use Gloo backend, but PP typically uses NCCL.
                        # Need to check if CPU tensors are passed through PP.
                        # Adding warning logging for now (strengthen error handling if needed)
                        logger.warning(f"Attempting to receive CPU tensor ({key}) in PP group ({self.unique_name}). device group backend: {torch.distributed.get_backend(self.device_group)}")
                        # Use CPU group (Gloo) for receiving CPU tensors
        ...
        # Broadcast the received dictionary to the TP group
        # broadcast_tensor_dict's src parameter is the rank within the TP group
        # The driver is TP rank 0
        if tp_group.world_size > 1:
            # broadcast_tensor_dict: driver provides dict, others provide None
            # i.e., sender: tensor dict, receiver: None as input
        ...
        # If we reached here, received_tensor_dict must not be None
...
def initialize_hetero_model_parallel(
    ...
    # Initialize global ParallelStrategy
    global ParallelStrategy
    ParallelStrategy = parallel_strategy

    # Manually assign groups instead of using torch.reshape or similar
    group_ranks = []
    start_rank = 0
    ...
    # Final verification after all TP groups initialized
    ...
    # Driver workers are the first rank in each TP group
    driver_worker_ranks = [group_ranks[x][0] for x in range(len(group_ranks))]
    group_ranks = [[]] # Driver workers are in index 0; rest are single-rank groups
    start_rank = 0
    for tp_group_size in parallel_strategy:
    ...
    global _DP
    assert _DP is None, ("data parallel group is already initialized")
    # DP not supported, initialize with placeholder
    group_ranks = [[x] for x in range(world_size)]
    _DP = init_model_parallel_group(group_ranks,
                                    get_world_group().local_rank,
                                    backend,
...
# New function to initialize heterogeneous TP parallelism per stage
def ensure_hetero_model_parallel_initialized(
        parallel_strategy: List[int],
        backend: Optional[str] = None) -> None:
    """Extends ensure_model_parallel_initialized to support uneven group partitioning.
    Designed to be mostly identical to the original function.
    """
    backend = backend or torch.distributed.get_backend(
        get_world_group().device_group)
    ...
    # PP check is not performed currently
...
# Utility functions to determine stage membership for a given rank
