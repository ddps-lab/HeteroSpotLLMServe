"""
Offline benchmark — Warmup baseline (Qwen3-32B, Scenario A)
...
Remaining nodes (not in ShuntServe config):
  - spot_g5_12xlarge 1 → P5 (64 layers, pp=[4])
  - spot_g5_12xlarge 2 → P6 (64 layers, pp=[4])
  - on_demand_g6e_xlarge 1,2,3,4 → P7 (16+16+16+16, pp=[1,1,1,1])
"""
import asyncio
...
EXTRA_PIPELINES = [
    {
        "label": "WU-P5",
        "predicted_throughput_rps": 0.466,
        "config": {
    ...
    },
    {
        ...
        "node_layer_mapping": [
            ["spot_g5_12xlarge_node_ip_2", 64],
        ],
    },
    {
        "label": "WU-P7",
        "predicted_throughput_rps": 5.865,
        "config": {
            "model_name": MODEL_NAME,
            "total_num_layers": 64,
            "gpu_memory_utilization": 0.85,
            ...
            "model_source": "s3",
            "s3_path": S3_PATH,
            "num_gpu_blocks": 19278,
            "max_batch_size": 309,
        },
        "node_layer_mapping": [
            ["on_demand_g6e_xlarge_node_ip_1", 16],
            ["on_demand_g6e_xlarge_node_ip_2", 16],
            ["on_demand_g6e_xlarge_node_ip_3", 16],
            ["on_demand_g6e_xlarge_node_ip_4", 16],
        ],
    },
]
...
async def test_benchmark():
    ...
    try:
        ...
        metrics = await run_trace_benchmark(
            ...
            trace_output_prefix=f"spottolerance_offline_warmup_qwen3_32b_scenario_{SCENARIO}",
