...
OUTPUT_PATH = os.path.join(RESULTS_DIR, "hexgen_p2.json")
...
NODE_LAYER_MAPPING = [
    ...
    (g6e_xlarge_node_ip_4, int(_pipeline['stages'][0][STAGE_LAYER_COUNT_IDX])),
    # stage[1]: (spot)g6.12xlarge(half) (TP=2, 2 GPU)
    (g6_12xlarge_node_ip_1, int(_pipeline['stages'][1][STAGE_LAYER_COUNT_IDX])),
    # stage[2]: (spot)g6.12xlarge(half) (TP=2, 2 GPU)
    (g6_12xlarge_node_ip_1, int(_pipeline['stages'][2][STAGE_LAYER_COUNT_IDX])),
    # stage[3]: (spot)g6.12xlarge(half) (TP=2, 2 GPU)
    (g6_12xlarge_node_ip_2, int(_pipeline['stages'][3][STAGE_LAYER_COUNT_IDX])),
    # stage[4]: (spot)g6.12xlarge(half) (TP=2, 2 GPU)
    (g6_12xlarge_node_ip_2, int(_pipeline['stages'][4][STAGE_LAYER_COUNT_IDX])),
    # stage[5]: (spot)g6.12xlarge(half) (TP=2, 2 GPU)
    (g6_12xlarge_node_ip_3, int(_pipeline['stages'][5][STAGE_LAYER_COUNT_IDX])),
    # stage[6]: (spot)g6.12xlarge(half) (TP=2, 2 GPU)
    (g6_12xlarge_node_ip_3, int(_pipeline['stages'][6][STAGE_LAYER_COUNT_IDX])),
    # stage[7]: (spot)g5.xlarge (TP=1, 1 GPU)
    (g5_xlarge_node_ip_1, int(_pipeline['stages'][7][STAGE_LAYER_COUNT_IDX])),
]
...
async def test_benchmark():
    ...
    print("=" * 70)
    print(f"HexGen Pipeline 2 — Config loaded from {PREDICTED_FILE}")
