#!/usr/bin/env python3
"""
Debug tool — show how spot trace events map to switch_nodes calls (UnitTest8B).
...
with open(os.path.join(SCRIPT_DIR, "spot_trace_events.json")) as f:
...
def short(name: str) -> str:
    ...
    return name.replace("_node_ip_", "#").replace("_xlarge", "")
...
def main():
    ...
    # ── Initial state ─────────────────────────────────────────────────
    print("=" * 80)
    print(f"  UnitTest8B — {pipelines_data['model']}")
    ...
    # any on_demand node in the initial config implies its spot counterpart is "interrupted"
    interrupted_spots = {}  # spot_name → on_demand_name
    ...
    for time_min in sorted(grouped_events.keys()):
        ...
        for event in grouped_events[time_min]:
            ...
            print(f"  {etype.upper()}: {[short(n) for n in instances]}{note}")
