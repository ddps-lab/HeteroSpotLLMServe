"""
Performance Estimation Benchmark: meta-llama/Llama-3.2-3B
Instance: p4d.24xlarge (A100_40GB ×8)
...
OUTPUT_PATH = os.path.join(RESULTS_DIR, "bench_p4d_24xlarge_tp1_pp8.json")
...
# Fill in the IP address of the p4d.24xlarge node.
# All 8 GPUs are on a single physical node.
NODE_IP = ""  # ← Set this to the node IP

NODE_LAYER_MAPPING = [
    # stage[0]: 4 layers (TP=1)
    (NODE_IP, 4),
    # stage[1]: 4 layers (TP=1)
    (NODE_IP, 4),
    # stage[2]: 4 layers (TP=1)
    (NODE_IP, 4),
    # stage[3]: 4 layers (TP=1)
    (NODE_IP, 4),
    # stage[4]: 3 layers (TP=1)
    (NODE_IP, 3),
    # stage[5]: 3 layers (TP=1)
    (NODE_IP, 3),
    # stage[6]: 3 layers (TP=1)
    (NODE_IP, 3),
    # stage[7]: 3 layers (TP=1)
    (NODE_IP, 3),
]
...
async def test_benchmark():
    ...
    print(f"  Instance: p4d.24xlarge (A100_40GB ×8)")
