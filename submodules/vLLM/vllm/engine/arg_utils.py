...
class EngineArgs:
    ...
    pipeline_parallel_size: int = 1 # Should be removed but kept to avoid potential errors
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # Added parallel_strategy argument
        parser.add_argument(
            '--parallel-strategy',
            type=int,       # List element type is integer
            nargs='+',      # Accept one or more arguments
            default=[1],    # Default provided as a list
            help=('Parallel strategy for the model. '
                  'Example: --parallel-strategy 4 2 means stage 0 has 4 '
                  'shards and stage 1 has 2 shards.')
        )
        # These arguments are deprecated but kept because not all code paths
        # have been updated to use parallel_strategy exclusively.
    ...
    def create_engine_config(
        ...
        parallel_config = ParallelConfig(
            # No longer passing pp and tp directly
            # pipeline_parallel_size=self.pipeline_parallel_size,
            # tensor_parallel_size=self.tensor_parallel_size,
            parallel_strategy=self.parallel_strategy, # Now using parallel_strategy instead
