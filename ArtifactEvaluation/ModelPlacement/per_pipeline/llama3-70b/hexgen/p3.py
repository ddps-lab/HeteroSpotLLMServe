...
8 stages: 7×g5.xlarge(TP=1) + 1×g6e.xlarge(TP=1)
Layers=[8,9,9,9,9,9,18,9]

Physical node allocation:
  g5.12xlarge #1 (4 A10G GPUs): stages 0,1,2,3 (each TP=1) = 4 GPUs
  g5.12xlarge #2 (3 of 4 A10G GPUs): stages 4,5,7 (each TP=1) = 3 GPUs
    ※ HX-P1 stage 7 originally shared this node, but was moved to a
       standalone g5.xlarge to avoid Ray conflicts. 1 GPU unused here.
  g6e.xlarge #4: stage 6 (TP=1) = 1 GPU
"""
import asyncio
...
# ─── Node assignment ─────────────────────────────────────────────────
# g5.12xlarge #1 (4 GPUs total):
#   stages 0,1,2,3 — each TP=1
#
# g5.12xlarge #2 (3 of 4 GPUs, 1 unused):
#   stages 4,5 — each TP=1
#   stage 7 — TP=1
#   (HX-P1 stage 7 moved to standalone g5.xlarge — Ray conflict avoidance)
#
# g6e.xlarge #4:
#   stage 6 — TP=1

NODE_LAYER_MAPPING = [
    (g5_12xlarge_node_ip_1, int(_pipeline["stages"][0][STAGE_LAYER_COUNT_IDX])),  # stage 0: g5.xlarge TP=1
    (g5_12xlarge_node_ip_1, int(_pipeline["stages"][1][STAGE_LAYER_COUNT_IDX])),  # stage 1: g5.xlarge TP=1
    (g5_12xlarge_node_ip_1, int(_pipeline["stages"][2][STAGE_LAYER_COUNT_IDX])),  # stage 2: g5.xlarge TP=1
    (g5_12xlarge_node_ip_1, int(_pipeline["stages"][3][STAGE_LAYER_COUNT_IDX])),  # stage 3: g5.xlarge TP=1
    (g5_12xlarge_node_ip_2, int(_pipeline["stages"][4][STAGE_LAYER_COUNT_IDX])),  # stage 4: g5.xlarge TP=1
    (g5_12xlarge_node_ip_2, int(_pipeline["stages"][5][STAGE_LAYER_COUNT_IDX])),  # stage 5: g5.xlarge TP=1
    (g6e_xlarge_node_ip_4,  int(_pipeline["stages"][6][STAGE_LAYER_COUNT_IDX])),  # stage 6: g6e.xlarge TP=1
    (g5_12xlarge_node_ip_2, int(_pipeline["stages"][7][STAGE_LAYER_COUNT_IDX])),  # stage 7: g5.xlarge TP=1
]
