...
class ModelConfig:
    ...
    def verify_with_parallel_config(
        ...
        # Must now check all tensor_parallel_size values in parallel_strategy
    ...
    def get_layers_start_end_indices(
        ...
        if self.hf_text_config.model_type == "deepseek_mtp":
            ...
            total_num_hidden_layers = getattr(self.hf_text_config,
                                              "num_hidden_layers", 0)
        # the layout order is: DP x PP x TP (no longer applies)
        # pp_rank = (parallel_config.rank // parallel_config.tensor_parallel_size
        #            ) % parallel_config.pipeline_parallel_size
        pp_rank = parallel_config.pipeline_parallel_rank # Use the new pipeline_parallel_rank property
...
class ParallelConfig:
    ...
    parallel_strategy: List[int] = field(default_factory=lambda: [1]) # Added for heterogeneous parallelism
    # tensor_parallel_size is now a computed property
    ...
    # Used for tensor manager access
    local_rank: int=-1
    ...
    def compute_hash(self):
        ...
        """
        factors: list[Any] = []
        # Changed hash computation factor from tp/pp to parallel_strategy
    ...
    def __post_init__(self) -> None:
        self.world_size = sum(self.parallel_strategy) # Modified world_size computation
        self.pipeline_parallel_size = len(self.parallel_strategy) # pipeline_parallel_size is now computed internally, no longer an input
    ...
    # Replace the old tensor_parallel_size field with a property
    @property
    ...
    # Also need to expose pipeline_parallel_rank as a property
    @property
