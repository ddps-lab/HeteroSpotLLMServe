...
EST_FILE = os.path.join(os.path.dirname(__file__), "..", "results",
                        "est_g6_48xlarge_tp1_pp8.json")
...
OUTPUT_PATH = os.path.join(RESULTS_DIR, "bench_g6_48xlarge_tp1_pp8.json")

# ─── Node assignment (manual) ────────────────────────────────────────
# Fill in the IP address of the g6.48xlarge node.
# All 8 GPUs are on a single physical node.
NODE_IP = ""  # ← Set this to the node IP

NODE_LAYER_MAPPING = [
    # stage[0]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[1]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[2]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[3]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[4]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[5]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[6]: 10 layers (TP=1)
    (NODE_IP, 10),
    # stage[7]: 10 layers (TP=1)
    (NODE_IP, 10),
]


def save_results(metrics, output_path, extra=None):
...
async def test_benchmark():
    ...
    print("=" * 70)
    print(f"Performance Estimation Benchmark")
    print(f"  Model: {MODEL_NAME}")
    print(f"  Instance: g6.48xlarge (L4 ×8)")
    print(f"  Strategy: TP=1, PP=8 → {PARALLEL_STRATEGY}")
    ...
    try:
        logger.info("Waiting for pipeline creation...")
        await pipeline_task
        ...
        save_results(metrics, OUTPUT_PATH, extra={
            "model": MODEL_NAME,
